...
async function main() {
  ...
  try {
    ...
    assert.match(await text("parsing-note"), /cases across/);
    assert.deepEqual(await evaluate('[...document.querySelectorAll(".parse-category-section")].map(s => s.id)'),
      ["parse-category-core", "parse-category-intrinsics", "parse-category-experimental"]);
    ...
    assert.equal(await evaluate('document.querySelectorAll(".parse-operation").length'), data.catalog.operations.length);
    assert.ok(await evaluate('Math.abs(document.getElementById("parse-category-intrinsics").getBoundingClientRect().top - 90) < 3'));
    await send("Page.reload");
    await until('document.getElementById("parse-category-intrinsics") && window.scrollY > 0');
    assert.equal(await evaluate('document.querySelectorAll(".parse-operation").length'), data.catalog.operations.length);
    assert.equal(await evaluate('getComputedStyle(document.querySelector(".action-panel")).display'), "none");
    await choose("parse-status", "gaps"); await choose("parse-search", "llvm.add");
    assert.ok(await evaluate('document.querySelectorAll(".parse-operation").length > 0'));
    await send("Page.reload");
    await until('document.getElementById("parse-search")?.value === "llvm.add"');
    assert.equal(await evaluate('document.getElementById("parse-status").value'), "gaps");
    const addSelector = '.parse-operation[data-operation="llvm.add"]';
    const add = data.parsing.at(-1).results.find(r => r.operation === "llvm.add");
    for (const form of ["scalar", "vector"]) {
      const cases = add.cases.filter(c => c.form === form), passed = cases.filter(c => c.strict.status === "parsed").length;
      const cell = await evaluate(`document.querySelector(${JSON.stringify(addSelector + ` [data-form="${form}"]`)}).textContent`);
      assert.ok(cell.startsWith(`${passed}/${cases.length} parse`));
    }
    ...
    await evaluate('document.querySelector("#parsing-operations .parse-examples").open = true');
    await evaluate('document.querySelector("#parsing-operations .parse-case").open = true');
    assert.ok(await evaluate('document.querySelector(".parsing-table").scrollHeight <= document.querySelector(".parsing-table").clientHeight'), "Parser evidence should expand the page without an inner vertical scrollbar");
    assert.match(await evaluate('document.querySelector(".parse-input").textContent'), /"llvm.add"/);
    assert.deepEqual(await evaluate('[...document.querySelectorAll(\'.parse-operation[data-operation="llvm.add"] .parse-case-heading\')].map(h => h.textContent)'), ["Scalar cases", "Vector cases"]);
    const inputLink = await evaluate('document.querySelector(".parse-case .parse-links a").getAttribute("href")');
    assert.ok(fs.existsSync(path.join(site, inputLink)), "Parser input download is missing");
    await evaluate('document.querySelector(".parse-case .parse-links a:nth-child(2)").click()');
    ...
    assert.match(await evaluate('document.querySelector(".parse-operation [data-form=scalar]").textContent'), /1\/1 parse/);
    assert.equal(await evaluate('document.querySelector(".parse-operation [data-form=vector]").textContent'), "Not tested");
    assert.match(await text("parsing-note"), new RegExp(`${data.catalog.operations.length} cases across`));
    await choose("parse-run", parserRun);
    await choose("parse-search", "llvm.intr.ceil"); await choose("parse-status", "all");
    assert.equal(await evaluate('document.querySelectorAll(".parse-operation").length'), 1);
    assert.match(await evaluate('document.querySelector(".parse-operation [data-form=scalar]").textContent'), /0\/1 parse/);
    await evaluate('document.querySelector("#parsing-operations .parse-examples").open = true');
    ...
    assert.equal(await evaluate('document.querySelector(".parse-operation [data-form=vector]").textContent'), "Not tested");
    await send("Page.reload");
    await until('document.getElementById("parse-mode")?.value === "permissive"');
    assert.match(await evaluate('document.querySelector(".parse-operation [data-form=scalar]").textContent'), /1\/1 parse/);
